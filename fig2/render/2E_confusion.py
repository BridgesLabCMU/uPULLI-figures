#!/usr/bin/env python3
"""fig2 render — Panel 2E: all-features Random-Forest confusion matrix.

Renders FROM the bundled mean row-normalized confusion matrix. The balanced accuracy shown in the title
is the mean of the matrix diagonal (= mean per-class recall = the reported CV balanced accuracy).

Reads:  data/all_confusion_cv.csv
Writes: figures/2E_confusion.{png,svg}
"""
import sys
...
cm = pd.read_csv(config.TABLES / 'all_confusion_cv.csv', index_col=0)
...
plt.tight_layout()
out = config.ensure(config.FIGURES) / '2E_confusion'
