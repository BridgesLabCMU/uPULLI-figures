#!/usr/bin/env python3
"""fig1 render: Panel 1E - DINOv2-embedding Random-Forest confusion matrix.

Renders FROM the bundled mean row-normalized confusion matrix. Balanced accuracy (title) = mean of the
matrix diagonal (= mean per-class recall = the reported CV balanced accuracy).

Reads:  data/embeddings_confusion_cv.csv
Writes: figures/1E_embeddingConfusion.{png,svg}
"""
import sys
...
cm = pd.read_csv(config.TABLES / 'embeddings_confusion_cv.csv', index_col=0)
