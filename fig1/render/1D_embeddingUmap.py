#!/usr/bin/env python3
"""fig1 render: Panel 1D - training-set UMAP from DINOv2 embeddings, colored by mutant.

Renders FROM the bundled coordinates table (euclid representation; the CSV also holds cosine).

Reads:  data/trainingEmbeddingUmap_coords.csv
Writes: figures/1D_embeddingUmap.{png,svg}
"""
import sys
...
df = pd.read_csv(config.TABLES / 'trainingEmbeddingUmap_coords.csv')
...
ax.set_title('DINOv2 Embeddings (euclidean)')
ax.set_xlabel('UMAP 1'); ax.set_ylabel('UMAP 2')
ax.legend(loc='center left', bbox_to_anchor=(1.02, 0.5), frameon=False)
plt.tight_layout()
out = config.ensure(config.FIGURES) / '1D_embeddingUmap'
