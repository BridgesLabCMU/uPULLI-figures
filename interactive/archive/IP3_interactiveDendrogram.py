#!/usr/bin/env python3
"""interactive build — Interactive Plot 3: dendrogram + heatmap of the reimaging atlas.

The interactive counterpart of Fig. S6A: the same Ward dendrogram over PCA-50 per-mutant centroids
and the same z-scored peak-biofilm-frame heatmap, but every mutant is clickable and opens the peak
biofilm biomass image of ALL of its replicates side by side — so the reader can see the genotype,
its position in the clustering, and the raw images the features came from in one place.
...
Reads:  data/fullAtlas_pcaLinkage_{linkage.npy, cluster_order.csv},
        data/fullAtlas_peakBiomass_featureMatrix.csv, data/wells.csv, thumbnail pack (config.input)
Writes: figures/interactivePlot3_dendrogramHeatmap.html

Usage:  python build/IP3_interactiveDendrogram.py
"""
import sys
import json
import argparse
...
ap.add_argument('--timelapse', action='store_true',
                help='gallery images follow the time slider (few replicates, every frame) instead '
                     'of showing every replicate at its peak-biomass frame')
args = ap.parse_args()

VMIN, VMAX = -3, 3
outHtml = config.ensure(config.FIGURES / ('interactivePlot3b_dendrogramTimelapse.html' if args.timelapse
                                          else 'interactivePlot3_dendrogramHeatmap.html'))
...
# ── replicate thumbnails, grouped under their mutant ──────────────────────────
# Two mutually exclusive modes, because their product is not shippable (all wells x all frames is
# ~330 MB of JPEG, ~440 MB base64):
#   default    every replicate, one image each (its peak-biomass frame)  -> ~15 MB
#   --timelapse  a few representative replicates, but every frame, so the gallery follows the
#                time slider                                             -> ~40 MB
...
if args.timelapse:
    ...
    for mutant, grp in wells.groupby('mutant'):
        idxs = []
        for _, r in grp.sort_values('wellId').iterrows():
            jpg = packDir / str(r['file'])
            if not jpg.exists():
                missing += 1
                continue
            idxs.append({'t': len(thumbList), 'w': f"{r['plateId']} · {r['wellId']}",
                         'pf': int(r['peakFrame']), 'bio': round(float(r['peakBiomass']), 5), 'fr': None})
            thumbList.append('data:image/jpeg;base64,' + base64.b64encode(jpg.read_bytes()).decode())
        byMutant[str(mutant)] = idxs
...
<div id="main">
  <div id="plot">
    <canvas id="cv"></canvas>
    <div id="tooltip"></div>
    <div id="hint">scroll=zoom · drag=pan · dblclick=fit</div>
  </div>
...
cv.addEventListener('wheel', e => {{
  ...
  scale *= f; ox = sx - wxp * scale; oy = sy - wyp * scale; draw();
}}, {{ passive: false }});
